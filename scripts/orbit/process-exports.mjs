...
/**
 * Builds workspace export manifests for pending Orbit export jobs.
...
const response = await request("/rest/v1/rpc/process_pending_export_jobs", {
  method: "POST",
  headers: {
...
const count = await expectStatus(response, [200], "orbit.process_pending_export_jobs");
console.log(`Processed ${count ?? 0} export job(s).`);
