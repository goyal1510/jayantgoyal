...
for (const sourceRoot of sourceRoots) {
  for (const file of walk(join(root, sourceRoot))) {
    ...
    if (clientDirective.test(source) && serviceRolePattern.test(source)) {
      violations.push(
        `${file}: client modules cannot access service-role Supabase code`,
      );
    }
    ...
    if (
      ...
      violations.push(
        `${file}: Portfolio cannot reference the service-role credential`,
      );
    }
  }
}
...
console.log(
  "Service-role boundaries are clean: credentials remain server-only and Portfolio-free.",
);
