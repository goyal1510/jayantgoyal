...
description: Run Jayant's multi-source software-engineering job pipeline across Wellfound, LinkedIn, and company career pages. Use for job discovery, fit ranking, application preparation, employee or recruiter research, referral drafts, confirmed submissions, nightly response checks, or maintaining the private Admin career tracker.
---
...
Search Wellfound, LinkedIn Jobs, and official company career pages. Prefer official career pages as the canonical application URL when the same role appears elsewhere. Use source identifiers and URLs to deduplicate both the current run and the Admin tracker.
...
3. Search all three sources and shortlist up to ten strong new matches.
...
6. Research up to three relevant people per company: recruiter, hiring manager, founder, or engineer on the likely team. Use public professional profiles. Do not bypass access controls, infer private email addresses, or use data-brokered contact information.
7. Draft one personalized referral or introduction message for the strongest contact. Make the ask small and honest; never imply a relationship or referral that does not exist.
...
2. Inspect Wellfound, LinkedIn, official applicant portals, and relevant inbox or thread views using the authenticated browser.
