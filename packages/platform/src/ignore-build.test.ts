...
describe("Vercel ignored build detection", () => {
  it("builds only the application changed in the deployment range", () => {
    ...
    appendFileSync(join(repository, "apps/studio/app.ts"), "changed\n");
  ...
  it("detects shared changes hidden behind a final documentation commit", () => {
    const { repository, base } = createRepository();
    appendFileSync(join(repository, "packages/ui/index.ts"), "changed\n");
    ...
    expect(runDetector(repository, "apps/studio", base, head).status).toBe(1);
    expect(runDetector(repository, "apps/admin", base, head).status).toBe(1);
  });
  ...
  it("skips a documentation-only deployment range", () => {
    ...
    expect(runDetector(repository, "apps/studio", base, head).status).toBe(0);
  });
  ...
  it("builds safely when the previous deployment SHA is missing", () => {
    ...
    const result = runDetector(repository, "apps/studio", undefined, base);
  ...
  it("builds safely when the previous deployment SHA is unavailable", () => {
    ...
    const result = runDetector(repository, "apps/studio", "missing", base);
