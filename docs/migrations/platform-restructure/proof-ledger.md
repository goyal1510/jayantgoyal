...
- Cloudflare stale Vercel records for `accounts`, `admin-employee`, `auth`, and `employee` were removed on the user's explicit instruction before implementation began. Additive DNS-only CNAME records now route `portfolio` and `studio` to their exact Vercel-recommended targets; apex, `www`, Admin, commerce, and mail records were not changed.
...
- Residual risks:
  ...
  - Stable staging domains remain pending.
