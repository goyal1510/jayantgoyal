...
- Consequences: when both names exist, the platform name wins and legacy state cannot resurrect an invalid platform session. Failed promotion can serve the already validated legacy client for that request, while invalid legacy state is ignored. Server Components on the first promotion request read its original validated legacy family because response cookies are not visible in the request snapshot; subsequent requests prefer the new platform family. Ordinary logout is explicitly local; global logout requires an explicit action. Cross-subdomain proof remains user-owned Production validation under ADR-008.
...
- Consequences: provider infrastructure can be deployed and tested manually without making Auth canonical. Cloudflare DNS now points Auth to Vercel, while the separate Studio/Admin owner flag remains `legacy`, preventing a login-traffic cutover before the first Auth deployment and acceptance. Generated Preview and Production acceptance remain user-owned under ADR-008.
