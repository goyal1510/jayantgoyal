...
| `pnpm check:identity`       | Public identity, package scope, and literal ownership stay fixed |
| `pnpm check:seo`            | Root/page metadata and tool-description contracts remain valid   |
...
GitHub Actions runs architecture, identity, brand asset, SEO, service-role,
source health, dead-code, documentation, lint, type, test, and complete build
checks for pull requests and pushes to `main`. Direct solo development still
requires the same gates before pushing.
