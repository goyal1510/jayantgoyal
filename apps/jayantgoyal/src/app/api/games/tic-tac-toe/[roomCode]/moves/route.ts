...
export async function POST(
  ...
  if (typeof rawCell !== "number" || !Number.isInteger(rawCell) || rawCell < 0 || rawCell > 8 || !isSafeJsonPayload(movePayload)) {
    return NextResponse.json({ error: "A valid cell is required." }, { status: 400 })
  }
  ...
  if (sessionError || !session) {
    return NextResponse.json({ error: "Tic Tac Toe room not found." }, { status: 404 })
  }

  if (session.status !== "active") {
    return NextResponse.json({ error: "Tic Tac Toe room is not active." }, { status: 409 })
  }
  ...
  const mark = participant.seat === "X" || participant.seat === "O" ? participant.seat as TicTacToeMark : null
  ...
  if (currentState.winner || currentState.isDraw || currentState.board[cell]) {
    return NextResponse.json({ error: "That cell is not playable." }, { status: 400 })
  }
  ...
  const completion = winner
    ? { outcome: "win", winnerParticipantId: participant.id, winner: winner.winner }
    : isDraw
      ? { outcome: "draw" }
      : null
  ...
  if (moveError) {
    console.error("Error recording Tic Tac Toe move:", moveError)
    return NextResponse.json({ error: "Unable to record Tic Tac Toe move." }, { status: 500 })
  }

  if (completion) {
    await supabase.schema("jg_app").from("game_hub_session_results").upsert({
      session_id: session.id,
      winner_participant_id: completion.outcome === "win" ? participant.id : null,
  ...
  await supabase
    ...
    .update({
      ...
      winner_participant_id: completion?.outcome === "win" ? participant.id : session.winner_participant_id,
