...
export function ActivityStats({ currentMonth }: ActivityStatsProps) {
  ...
  React.useEffect(() => {
    ...
    const loadStats = async () => {
      try {
        setIsLoading(true)
        const response = await fetch(`/api/activity-tracker/stats?month=${currentMonth}`, {
          cache: "no-store",
        })
      ...
      <Card>
        <CardContent className="flex flex-col items-center justify-center py-12">
          <p className="text-muted-foreground text-center">
            No activities yet. Create activities using the &ldquo;Add Activity&rdquo; button above to see your performance metrics here.
          </p>
        </CardContent>
      </Card>
    )
