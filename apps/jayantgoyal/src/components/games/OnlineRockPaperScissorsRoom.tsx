...
import { ArrowLeft, Copy, Loader2, Share2, Swords, Users } from "lucide-react"
...
const CHOICE_META: Record<RpsChoice, { label: string; image: string }> = {
  rock: { label: "Rock", image: "/assets/games/Rock-Paper-Scissor/resources/rock.png" },
  paper: { label: "Paper", image: "/assets/games/Rock-Paper-Scissor/resources/paper.png" },
  scissors: { label: "Scissors", image: "/assets/games/Rock-Paper-Scissor/resources/scissors.png" },
}
...
export function OnlineRockPaperScissorsRoom({ roomCode }: { roomCode: string }) {
  ...
  const [submittingChoice, setSubmittingChoice] = React.useState<RpsChoice | null>(null)
  ...
  const playerOne = participants.find((participant) => participant.seat === "P1")
  const playerTwo = participants.find((participant) => participant.seat === "P2")
  ...
  const canChoose = roomActive && !!mySeat && !myPendingChoice && !submittingChoice
  ...
  const copyInvite = async () => {
    await navigator.clipboard.writeText(`${window.location.origin}/games/rock-paper-scissors/room/${roomCode}`)
  ...
  const submitChoice = async (choice: RpsChoice) => {
    ...
    const response = await fetch(`/api/games/rock-paper-scissors/${roomCode}/moves`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      ...
      <Card className="overflow-hidden border-fuchsia-200 bg-[radial-gradient(circle_at_top_left,#fae8ff,transparent_32%),linear-gradient(135deg,#faf5ff,#fdf2f8)] dark:border-fuchsia-950 dark:bg-[linear-gradient(135deg,#2e1065,#4a044e)]">
        <CardHeader className="flex flex-row items-center justify-between gap-3">
          <CardTitle className="flex min-w-0 items-center gap-2">
            <Swords className="h-5 w-5 shrink-0 text-fuchsia-600" />
            <span className="truncate">RPS Room {roomCode}</span>
          </CardTitle>
          <div className="flex items-center gap-2">
            <Button variant="outline" size="icon" onClick={() => router.push("/games/rock-paper-scissors")} aria-label="Back to Rock Paper Scissors">
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            {RPS_CHOICES.map((choice) => (
              ...
              >
                ...
                <span className="text-sm font-medium">{CHOICE_META[choice].label}</span>
                {submittingChoice === choice && <Loader2 className="h-4 w-4 animate-spin" />}
              </Button>
            ))}
          </div>
          ...
          {state.lastRound && (
            <div className="rounded-lg border bg-background/80 p-3 text-sm">
              ...
              <div className="mt-1 text-muted-foreground">
                P1 chose {CHOICE_META[state.lastRound.choices.P1].label}, P2 chose {CHOICE_META[state.lastRound.choices.P2].label}.
              </div>
              <div className="mt-1 font-semibold">
                {state.lastRound.outcome === "draw" ? "Draw" : `${state.lastRound.outcome} won the round`}
              </div>
            </div>
          )}
        </CardContent>
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              <div className="space-y-2">
                ...
                <Input id="rps-room-name" value={displayName} onChange={(event) => setDisplayName(event.target.value)} />
              </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-3 text-sm">
            ...
            <div className="grid grid-cols-2 gap-2">
              <div className="rounded-lg border p-3">
                <div className="text-xs uppercase tracking-wide text-muted-foreground">P1</div>
                <div className="truncate font-medium">{playerOne?.display_name ?? "Open"}</div>
                <div className="mt-1 text-xl font-semibold">{state.scores.P1}</div>
              </div>
              <div className="rounded-lg border p-3">
                <div className="text-xs uppercase tracking-wide text-muted-foreground">P2</div>
                <div className="truncate font-medium">{playerTwo?.display_name ?? "Open"}</div>
                <div className="mt-1 text-xl font-semibold">{state.scores.P2}</div>
              </div>
            </div>
            <div className="rounded-lg border p-3">
              <div className="text-xs uppercase tracking-wide text-muted-foreground">Draws</div>
