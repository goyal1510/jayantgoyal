...
import { ArrowLeft, Copy, Loader2, Share2, SpellCheck2, Users } from "lucide-react"
...
import { parseWordleState, WORDLE_MAX_GUESSES, WORDLE_WORD_LENGTH, type WordleState } from "@/lib/games/wordle"
...
function coerceBundle(bundle: OnlineSessionBundle | null): {
  ...
  if (!bundle?.session) return { session: null, participants: [], result: null }
  return {
...
export function OnlineWordleRoom({ roomCode }: { roomCode: string }) {
  ...
  const canGuess = session?.status === "active" && Boolean(mySeat) && !myState?.done && !state.winner && !state.isDraw
  ...
  const copyInvite = async () => {
    await navigator.clipboard.writeText(`${window.location.origin}/games/wordle/room/${roomCode}`)
      ...
      <Card className="overflow-hidden border-emerald-200 bg-[radial-gradient(circle_at_top_left,#dcfce7,transparent_34%),linear-gradient(135deg,#f8fafc,#ecfdf5)] dark:border-emerald-900/70 dark:bg-[linear-gradient(135deg,#111827,#064e3b)]">
        <CardHeader className="flex flex-row items-center justify-between gap-3">
          <CardTitle className="flex min-w-0 items-center gap-2">
            <SpellCheck2 className="h-5 w-5 shrink-0 text-emerald-600" />
            <span className="truncate">Wordle Room {roomCode}</span>
          </CardTitle>
          <div className="flex items-center gap-2">
            <Button variant="outline" size="icon" onClick={() => router.push("/games/wordle")} aria-label="Back to Wordle">
        ...
        <CardContent className="space-y-4 pb-6">
          <div className="grid gap-4 lg:grid-cols-2">
            {(["P1", "P2"] as const).map((seat) => {
              ...
              return (
                <div key={seat} className={cn("rounded-lg border bg-background/80 p-3", mySeat === seat && "ring-2 ring-emerald-400")}>
                  <div className="mb-3 flex items-center justify-between gap-2">
                    <div>
                      <div className="text-xs uppercase tracking-wide text-muted-foreground">{seat}</div>
                      <div className="font-semibold">{participant?.display_name ?? "Open"}</div>
                    </div>
                    <div className="text-xs font-medium text-muted-foreground">
                      {player.won ? "Solved" : player.done ? "Done" : `${player.guesses.length}/${state.maxGuesses}`}
                    </div>
                  </div>
          ...
          <div className="mx-auto flex max-w-md gap-2">
            <Input
              value={guess}
              onChange={(event) => setGuess(event.target.value.slice(0, WORDLE_WORD_LENGTH).toLowerCase())}
              onKeyDown={(event) => {
              ...
              className="uppercase"
            />
            <Button onClick={() => void submitGuess()} disabled={!canGuess || submittingGuess}>
              {submittingGuess ? <Loader2 className="h-4 w-4 animate-spin" /> : "Guess"}
            </Button>
          </div>
        </CardContent>
      </Card>
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              <div className="space-y-2">
                ...
                <Input id="wordle-room-name" value={displayName} onChange={(event) => setDisplayName(event.target.value)} />
              </div>
              <Button onClick={joinRoom} disabled={joining} className="w-full">
                {joining ? <Loader2 className="h-4 w-4 animate-spin" /> : "Join as P2"}
              </Button>
            </CardContent>
          </Card>
        )}
          ...
          <CardContent className="space-y-3 text-sm">
            ...
            {solution && (
              <div className="rounded-lg border p-3">
                ...
                <div className="font-mono text-lg font-semibold uppercase tracking-[0.2em]">{solution}</div>
              </div>
            )}
            <div className="rounded-lg border p-3">
