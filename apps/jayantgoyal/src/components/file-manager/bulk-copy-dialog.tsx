...
export function BulkCopyDialog({
  ...
  const copyableFiles = files.filter((file) => !file.is_directory)
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (copyableFiles.length === 0) return
    ...
    try {
      ...
      for (const file of copyableFiles) {
        ...
        if (!response.ok) {
          failures.push(
            `${file.display_name || file.file_name}: ${data.error || "Copy failed"}`
          )
        }
      }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Copy {copyableFiles.length} selected file
            {copyableFiles.length === 1 ? "" : "s"} to another folder.
          </DialogDescription>
        </DialogHeader>
        <form onSubmit={handleSubmit}>
          ...
          <DialogFooter className="flex-col-reverse gap-2 sm:flex-row">
            ...
            <Button
              type="submit"
              disabled={loading || copyableFiles.length === 0}
              className="w-full sm:w-auto"
            >
