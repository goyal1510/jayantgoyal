...
export function AppSidebar({
  surface = "legacy",
  ...props
}: React.ComponentProps<typeof Sidebar> & { surface?: PlatformSurface }) {
  const pathname = usePathname();
  const apps = React.useMemo(() => getSurfaceApps(surface), [surface]);
  ...
  const rootAppId = surface === "studio" ? "studio-home" : "portfolio";
  const { activeAppId, activeNavId } = useActiveApp(pathname, apps, rootAppId);

  // Portfolio scroll tracking
  const portfolioSectionIds = React.useMemo(
    () =>
      apps
        .find((app) => app.id === "portfolio")
        ?.navItems.map((item) => item.id) ?? [],
    [apps],
  );
  const portfolioActiveSection = useScrollTracking(
    portfolioSectionIds,
    surface === "legacy" && activeAppId === "portfolio",
  );
  const finalActiveNavId =
    activeAppId === "portfolio" ? portfolioActiveSection : activeNavId;
  ...
  return (
    <Sidebar collapsible="icon" {...props}>
      <SidebarHeader>
        <TeamSwitcher
          brand={{
            name: surface === "studio" ? "JG Studio" : "JG",
            logo: LayoutGrid,
          }}
        />
      </SidebarHeader>
