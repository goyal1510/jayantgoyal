...
export function generateCalculationPDF(calc: CalculationWithDenominations): void {
  ...
  const cards = [
    ...
    { label: "Denominations", value: String(sortedDenoms.filter((d) => d.count !== 0).length) },
  ...
  sortedDenoms.forEach((denom, i) => {
    ...
    doc.setFontSize(9)
    doc.text(`\u20B9${denom.denomination.toLocaleString("en-IN")}`, col1X + 4, y + 5.5)
  ...
  doc.text(`Grand Total: ${formatCurrency(grandTotal)}`, pageWidth - margin, y, {
    align: "right",
  })
  doc.setTextColor(0, 0, 0)
  ...
  doc.text(
    `Generated on ${new Date().toLocaleDateString("en-IN")} | jayantgoyal.com`,
    pageWidth / 2,
    pageHeight - 10,
    { align: "center" }
  )
