...
import { loadGithubActivityWorkspace } from "@/lib/portfolio-workspace";
import { ActivitySourceForm } from "../activity/activity-source-form";

export const metadata: Metadata = { title: "GitHub" };

export default async function GithubActivityWorkspacePage() {
  const supabase = await createSupabaseServerClient();
  const { hero: heroRecord, editorial } =
    await loadGithubActivityWorkspace(supabase);
  const githubUsername = heroRecord?.github_username ?? "";

  return (
    <div className="mx-auto w-full max-w-6xl space-y-8">
      <PortfolioWorkspaceHeader workspace="github_activity" />
      <SectionEditorialPanel sectionKey="github_activity" {...editorial} />
        ...
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-muted-foreground">
              Contribution history is collected from the public GitHub profile
              at render time. It is not duplicated as editable Portfolio
              content in the CMS.
            </p>
          </div>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <Card className="bg-muted/30">
          ...
          <CardContent className="text-sm text-muted-foreground">
            Edit the heading copy here. Keep GitHub metrics derived so the
