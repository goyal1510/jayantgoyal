...
import { ArrowLeft, Check, Copy, Loader2, Share2, Users, X, Zap } from "lucide-react"
...
import { parseDareXState, type DareXAction, type DareXState } from "@/lib/games/dare-x"
...
export function OnlineDareXRoom({ roomCode }: { roomCode: string }) {
  ...
  const [submittingAction, setSubmittingAction] = React.useState<DareXAction | null>(null)
  ...
  const copyInvite = async () => {
    await navigator.clipboard.writeText(`${window.location.origin}/games/dare-x/room/${roomCode}`)
      ...
      <Card className="overflow-hidden border-rose-200 bg-[radial-gradient(circle_at_top_left,#ffe4e6,transparent_32%),linear-gradient(135deg,#fff7ed,#fff1f2)] dark:border-rose-950 dark:bg-[linear-gradient(135deg,#451a03,#4c0519)]">
        <CardHeader className="flex flex-row items-center justify-between gap-3">
          <CardTitle className="flex min-w-0 items-center gap-2">
            <Zap className="h-5 w-5 shrink-0 text-rose-600" />
            <span className="truncate">Dare X Room {roomCode}</span>
          </CardTitle>
          <div className="flex items-center gap-2">
            <Button variant="outline" size="icon" onClick={() => router.push("/games/dare-x")} aria-label="Back to Dare X">
        ...
        <CardContent className="space-y-4 pb-6">
          <div className="rounded-xl border bg-background/85 p-4">
            <div className="flex items-center justify-between gap-3 text-xs text-muted-foreground">
              ...
              <span>Turn {state.round} / {state.targetRounds}</span>
            </div>
            <div className="mt-2 text-2xl font-semibold">{currentParticipant?.display_name ?? "Waiting"}</div>
          </div>

          <div className="rounded-xl border bg-background/85 p-5">
            <div className="text-xs uppercase tracking-wide text-muted-foreground">Current dare</div>
            <div className="mt-3 min-h-[110px] text-xl font-semibold leading-relaxed">
              {state.currentDare ?? (isMyTurn ? "Get a dare to start your turn." : "Waiting for the current player.")}
            </div>
          </div>

          <div className="flex flex-wrap gap-2">
            <Button onClick={() => void submitAction("get_dare")} disabled={!canGetDare} className="min-w-[140px] flex-1">
              {submittingAction === "get_dare" ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Zap className="mr-2 h-4 w-4" />}
              Get dare
            </Button>
            <Button variant="outline" onClick={() => void submitAction("done")} disabled={!canResolve} className="min-w-[140px] flex-1">
              <Check className="mr-2 h-4 w-4" />
              Done
            </Button>
            <Button variant="outline" onClick={() => void submitAction("not_done")} disabled={!canResolve} className="min-w-[140px] flex-1">
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              <div className="space-y-2">
                ...
                <Input id="dare-room-name" value={displayName} onChange={(event) => setDisplayName(event.target.value)} />
              </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-2 text-sm">
            ...
            {participants.map((participant) => {
              const completed = state.completed[participant.seat] ?? { done: [], skipped: [] }
              return (
                <div
                  key={participant.id}
                  className={cn(
                    "rounded-lg border p-3",
                    participant.seat === state.currentSeat && "border-rose-400 bg-rose-500/10"
                  )}
                >
                  <div className="flex items-center justify-between gap-2">
                    <div className="truncate font-medium">{participant.display_name}</div>
                    <div className="font-mono text-xs text-muted-foreground">{participant.seat}</div>
                  </div>
