...
import { ArrowLeft, Copy, Grid3X3, Loader2, Share2, Users } from "lucide-react"
...
import { parseTicTacToeState, type TicTacToeState } from "@/lib/games/tic-tac-toe"
...
export function OnlineTicTacToeRoom({ roomCode }: { roomCode: string }) {
  ...
  const copyInvite = async () => {
    await navigator.clipboard.writeText(`${window.location.origin}/games/tic-tac-toe/room/${roomCode}`)
    toast.success("Invite link copied")
  }

  const submitMove = async (cell: number) => {
    if (!isMyTurn || state.board[cell] || state.winner || state.isDraw || submittingMove) return
      ...
      <Card className="overflow-hidden border-sky-200 bg-[radial-gradient(circle_at_top_left,#e0f2fe,transparent_32%),linear-gradient(135deg,#f8fafc,#eff6ff)] dark:border-sky-900/70 dark:bg-[linear-gradient(135deg,#0f172a,#082f49)]">
        <CardHeader className="flex flex-row items-center justify-between gap-3">
          <CardTitle className="flex min-w-0 items-center gap-2">
            <Grid3X3 className="h-5 w-5 shrink-0 text-sky-600" />
            <span className="truncate">Tic Tac Toe Room {roomCode}</span>
          </CardTitle>
          <div className="flex items-center gap-2">
            <Button variant="outline" size="icon" onClick={() => router.push("/games/tic-tac-toe")} aria-label="Back to Tic Tac Toe">
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              <div className="space-y-2">
                ...
                <Input id="tic-room-name" value={displayName} onChange={(event) => setDisplayName(event.target.value)} />
              </div>
              <Button onClick={joinRoom} disabled={joining} className="w-full">
                {joining ? <Loader2 className="h-4 w-4 animate-spin" /> : "Join as O"}
              </Button>
            </CardContent>
          </Card>
        )}
        ...
        <Card>
          ...
          <CardContent className="space-y-3 text-sm">
            ...
            <div className="grid grid-cols-2 gap-2">
              <div className="rounded-lg border p-3">
                <div className="text-xs uppercase tracking-wide text-muted-foreground">X</div>
                <div className="truncate font-medium">{playerX?.display_name ?? "Open"}</div>
              </div>
              <div className="rounded-lg border p-3">
                <div className="text-xs uppercase tracking-wide text-muted-foreground">O</div>
                <div className="truncate font-medium">{playerO?.display_name ?? "Open"}</div>
              </div>
            </div>
            <div className="rounded-lg border p-3">
