...
import { ArrowLeft, Brain, Copy, Loader2, Share2, Users } from "lucide-react"
...
import { MEMORY_MATCH_DIFFICULTIES, parseMemoryMatchState, type MemoryMatchState } from "@/lib/games/memory-match"
...
export function OnlineMemoryMatchRoom({ roomCode }: { roomCode: string }) {
  ...
  const copyInvite = async () => {
    await navigator.clipboard.writeText(`${window.location.origin}/games/memory-match/room/${roomCode}`)
  ...
  const submitFlip = async (cardId: number) => {
    ...
    if (!card || card.flipped || card.matched || state.selectedCards.length >= 2) return
      ...
      <Card className="overflow-hidden border-pink-200 bg-[radial-gradient(circle_at_top_left,#fce7f3,transparent_34%),linear-gradient(135deg,#fff7ed,#fdf2f8)] dark:border-pink-900/70 dark:bg-[linear-gradient(135deg,#1f2937,#4c1d35)]">
        <CardHeader className="flex flex-row items-center justify-between gap-3">
          <CardTitle className="flex min-w-0 items-center gap-2">
            <Brain className="h-5 w-5 shrink-0 text-pink-600" />
            <span className="truncate">Memory Match Room {roomCode}</span>
          </CardTitle>
          <div className="flex items-center gap-2">
            <Button variant="outline" size="icon" onClick={() => router.push("/games/memory-match")} aria-label="Back to Memory Match">
          ...
          >
            {state.cards.map((card) => (
              <button
                ...
                disabled={!isMyTurn || card.flipped || card.matched || state.selectedCards.length >= 2 || submittingFlip || !!state.winner || state.isDraw}
                className={cn(
                  ...
                  state.lastMove?.cardIds.includes(card.id) && "ring-2 ring-pink-400"
                )}
      ...
      <div className="space-y-4">
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              <div className="space-y-2">
                ...
                <Input id="memory-room-name" value={displayName} onChange={(event) => setDisplayName(event.target.value)} />
              </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-3 text-sm">
            ...
            <div className="grid grid-cols-2 gap-2">
              <div className="rounded-lg border p-3">
                <div className="text-xs uppercase tracking-wide text-muted-foreground">Moves</div>
