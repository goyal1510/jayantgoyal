...
function getBreadcrumbItems(pathname: string): { name: string; url: string }[] {
  ...
  if (pathname.startsWith("/activity-tracker")) {
    items.push({ name: "Activity Tracker", url: `${SITE_URL}/activity-tracker/dashboard` });
    if (pathname.includes("/dashboard")) items.push({ name: "Dashboard", url: `${SITE_URL}${pathname}` });
    else if (pathname.includes("/tracker")) items.push({ name: "Tracker", url: `${SITE_URL}${pathname}` });
    else if (pathname.includes("/management")) items.push({ name: "Management", url: `${SITE_URL}${pathname}` });
    return items;
  }

  // Calculator routes
  if (pathname.startsWith("/calculator")) {
    items.push({ name: "Currency Calculator", url: `${SITE_URL}/calculator/new` });
    if (pathname === "/calculator/new") items.push({ name: "New", url: `${SITE_URL}${pathname}` });
    else if (pathname === "/calculator/history") items.push({ name: "History", url: `${SITE_URL}${pathname}` });
    return items;
  }
