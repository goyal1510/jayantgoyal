...
export async function GET(request: NextRequest) {
  try {
    ...
    const listQuery = buildFilteredQuery(CALCULATION_SELECT_COLUMNS, true);
    const dateQuery = buildFilteredQuery("created_at");
    const [listResult, dateResult] = await Promise.all([
      listQuery.order("created_at", { ascending: false }).range(from, to),
      dateQuery.order("created_at", { ascending: false }).range(0, 499),
    ]);
    const { data: calculations, error: calcError, count } = listResult;
    const { data: dateRows, error: dateError } = dateResult;
    ...
    const normalizeCalculation = (calc: unknown): CalculationRow | null => {
      if (
        !calc ||
        typeof calc !== "object" ||
        typeof (calc as { id?: unknown }).id !== "string"
      ) {
        return null;
      }
    ...
    const normalizeDenom = (denom: unknown): DenominationRow | null => {
      ...
      return {
        id: candidate.id,
        calculation_id:
          typeof candidate.calculation_id === "string"
            ? candidate.calculation_id
            : null,
        denomination:
          typeof candidate.denomination === "number"
            ? candidate.denomination
            : Number(candidate.denomination ?? 0),
        count:
          typeof candidate.count === "number"
            ? candidate.count
            : Number(candidate.count ?? 0),
        bundle_count:
          typeof candidate.bundle_count === "number"
            ? candidate.bundle_count
            : null,
        open_count:
          typeof candidate.open_count === "number"
    ...
    return NextResponse.json({
      ...
      pageSize,
      totalPages,
      availableDates: Array.from(availableDateSet).sort((a, b) =>
        a < b ? 1 : -1,
      ),
    });
  } catch (error) {
    console.error("Error in GET /api/calculator:", error);
    return NextResponse.json(
  ...
  try {
    ...
    const denominationsResult = denominationsToInsert.length
      ? await supabase
          ...
          .select(DENOMINATION_SELECT_COLUMNS)
      : { data: [], error: null };
    const { data: createdDenominations, error: denomError } =
      denominationsResult;
    ...
    console.error("Error in POST /api/calculator:", error);
    return NextResponse.json(
      { error: "Internal server error" },
      { status: 500 },
    );
