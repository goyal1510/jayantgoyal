...
export async function POST(
  ...
  if (sessionError || !session) {
    return NextResponse.json({ error: "Memory Match room not found." }, { status: 404 })
  }

  if (session.status !== "active") {
    return NextResponse.json({ error: "Memory Match room is not active." }, { status: 409 })
  }
  ...
  if (currentState.winner || currentState.isDraw || currentState.selectedCards.length >= 2) {
    return NextResponse.json({ error: "The board is not ready for another flip." }, { status: 409 })
  }

  const targetCard = currentState.cards.find((card) => card.id === cardId)
  if (!targetCard || targetCard.flipped || targetCard.matched || currentState.selectedCards.includes(cardId)) {
    return NextResponse.json({ error: "That card is not playable." }, { status: 400 })
  }
  ...
  if (moveError) {
    console.error("Error recording Memory Match flip:", moveError)
    return NextResponse.json({ error: "Unable to record Memory Match flip." }, { status: 500 })
  }
  ...
  await supabase
    ...
    .update({
      ...
      current_turn_participant_id: completion ? null : nextParticipantId,
      winner_participant_id: winnerParticipantId,
