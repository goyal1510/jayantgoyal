...
export async function POST(
  ...
  if (session.status !== "active") {
    return NextResponse.json(
      { error: "Dare X room is not active." },
      { status: 409 },
    );
  }
  ...
  const currentIndex = activeParticipants.findIndex(
    (item) => item.seat === participant.seat,
  );
  const nextParticipant =
    activeParticipants[(currentIndex + 1) % activeParticipants.length] ??
  ...
  if (action === "get_dare") {
    if (state.currentDare) {
      return NextResponse.json(
        { error: "Resolve the current dare first." },
        { status: 409 },
      );
    }
    ...
    const currentCompleted = state.completed[participant.seat] ?? {
      done: [],
      skipped: [],
    };
    const nextCompleted = {
  ...
  const { error: actionError } = await recordOnlineGameAction(supabase, {
    ...
    resultingState,
    nextTurnParticipantId: completion
      ? null
      : action === "get_dare"
        ? participant.id
  ...
  if (actionError) {
    console.error("Error recording Dare X action:", actionError);
    return NextResponse.json(
      {
        error:
          actionError.code === "P0001"
            ? "The game changed. Refresh and try again."
            : "Unable to record Dare X action.",
      },
