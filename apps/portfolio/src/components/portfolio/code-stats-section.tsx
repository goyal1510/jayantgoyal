...
export function CodeStatsSection({
  githubUsername,
  initialData,
}: CodeStatsSectionProps) {
  const [stats, setStats] = useState<GitHubLOCStats | null>(
    initialData ?? null,
  );
    ...
    async function fetchStats() {
      try {
        const res = await fetch(
          `/api/github-loc?username=${encodeURIComponent(githubUsername)}`,
        );
...
function LanguageBar({ breakdown }: { breakdown: LanguageLOCBreakdown[] }) {
  ...
  const otherPercentage = breakdown
    .slice(6)
