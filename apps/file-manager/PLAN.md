...
CREATE INDEX idx_files_path_prefix ON files(user_id, file_path text_pattern_ops) WHERE NOT is_deleted;

-- For listing files in a directory quickly
CREATE INDEX idx_files_user_parent_type ON files(user_id, parent_id, file_type) WHERE NOT is_deleted;
```
...
```sql
CREATE OR REPLACE FUNCTION get_directory_tree(
  p_user_id UUID,
...
```sql
CREATE OR REPLACE FUNCTION create_directory_path(
  p_user_id UUID,
...
```sql
CREATE OR REPLACE FUNCTION get_file_by_path(
  p_user_id UUID,
  p_file_path TEXT
)
...
INSERT INTO file_type_categories (type_name, display_name, allowed_mime_types, can_preview) VALUES
...
ALTER TABLE file_type_categories ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Anyone can view file types"
ON file_type_categories FOR SELECT
TO authenticated
USING (true);
```
...
BEGIN
  ...
  new_storage_path := generate_storage_path(p_user_id, new_file_path, file_record.file_name);
...
```sql
SELECT create_directory_path(auth.uid(), '/documents/personal/adhar');
```

### 2. List directory contents

```sql
SELECT * FROM list_directory(auth.uid(), '/documents/personal');
```
...
INSERT INTO files (
  ...
  auth.uid(),
  'private-files',
  generate_storage_path(auth.uid(), '/documents/personal/adhar/', 'front.pdf'),
  'front.pdf',
  'Aadhaar Front',
  'application/pdf',
  2048000,
  ...
  'pdf',
  false,
  (SELECT id FROM files WHERE user_id = auth.uid() AND file_path = '/documents/personal/adhar/')
);
```
...
```sql
SELECT * FROM get_directory_tree(auth.uid(), '/documents');
```
...
const { data: dirId } = await supabase.rpc('create_directory_path', {
  p_user_id: user.id,
...
const { data: dirContents } = await supabase.rpc('list_directory', {
  p_user_id: user.id,
